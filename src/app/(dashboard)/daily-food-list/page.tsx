    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {/* New Feature: Generate Food Request Letter */}
          <button
            onClick={() => {
              if (!updatedList || updatedList.entries.length === 0) {
                showToast('No students have been added to the Daily Food List. Please add students before generating the food request letter.');
                return;
              }
      ...
      {/* Food Request Letter Modal */}
      <FoodRequestLetterModal
        isOpen={showLetterModal}
        onClose={() => setShowLetterModal(false)}
        foodDate={selectedDate}
        studentsList={(updatedList?.entries ?? []).map(entry => {
          const s = students.find(st => st.id === entry.studentId);
          return {
            studentId: entry.studentId,
            name: s?.name ?? entry.studentId,
          };
        })}
      />
